...
#                 continue # 
    ...
    def get_ct(self, w= 50, c0 = .1, eps = 1e-2):
        i = array(self.tv[-w:])
        if len(i) < w:
            self.ct = c0
            return 0
    ...
    def rebin(self, rebin = 'auto' , bin_size = 10, ct = 'auto',c0 = 0.03, min_step = 40, **kw):
        if rebin == 'auto':
...
class netRads(netSimple):
    ...
    def save(nn,path, tv=1):
        ...
        for k in nn.elist: #edg:
            net['links'][lab(k)]={
                 u'end_points':int0(k[:2]).tolist(),
                 u'points':float16(nn.pts[int0(k[:2])]).tolist(),
                 u'radius':(k[2] if len(k)>2 else 1.),
                 u'weight':1.}
        json.dump(net,open(path+nn.gnam +'.json','w'))
        
        if tv: savetxt(path+nn.gnam +'-tv.txt',nn.tv)
...
def check_convergence(self, tol = .02, win = 500, **kw):
...
def iter_converge(self,its = 'auto', max_its = 8e4, draw=True, verbose=True, eps = 0.1, save_path =None, **kw):
