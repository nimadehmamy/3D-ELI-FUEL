...
class networkBase():
    def __init__(self, pts=[], edg=[],fixed=False, JSON=None, keep_paths=False, **kw):
        """ 
        pts: node positions
        edg: edge list
        ...
        self.params = {'links':
                               ...
                               'weighted': 0},
                               'nodes':
                               {'amplitude' : 5e2, 'radius': 5, 'weighted':0},
                              }
...
#                 continue # 
    ...
    def iter_all(self,it = 1, rebin = 'auto' , bin_size = 10, ct='auto', ellipsoid = False, **kw):
        """Calls
        ...
        for i in range(it):
            self.rebin(rebin = rebin, bin_size = bin_size, ct = ct)
    ...
    def get_ct(self, w= 50, c0 = .01, eps = 1e-3):
        i = array(self.tv[-w:])
        if len(i) < w:
            self.ct = c0
            return 0
    ...
    def rebin(self, rebin = 'auto' , bin_size = 10, ct = 'auto', min_step = 40):
        if rebin == 'auto':
            if self.it_num >= self.next_binning:
                ...
                for i in range(5): # at most 5 times
                    ...
                    if len(self.curr_keys) < self.links.max_workers or mx > PAIRS: # too many points in bins
                        ...
                        if self.bin_size > 2: # only make smaller if cell_size > 2*max(links.thickness)
                            self.bin_size /= 2.
                        else: print "[=]",
                        self.links.bin_rand(self.bin_size)
                ...
                self.next_binning += (10*(b-c))/(c+1) + min_step 
                print self.it_num,
                if ct == 'auto': 
                    self.get_ct()
                    print '|%.2g'%self.ct,
    ...
    class net_nodes():
        ...
        def dyn_setup(nodes):
            if not nodes.net.fixed:
                ...
                nodes.dp = nodes.net.f_node * nodes.net.dt
            else:
        ...
        def force_node_repel_brute(nodes):
            ...
            r0 = (nodes.degrees**1.)*nodes.radius #(nodes.degrees**.5)*nodes.radius
            th = r0 + r0[:,newaxis]
#             th = 2*nodes.radius
...
#             links.end_pts = array(links.idx.values(), dtype=int32) # to accelerate correcting endpoint forces
        ...
        def force_ext_ellipsoid_idx_multi(links,id):
            ...
            drh = dr / (tf.norm(dr, axis = -1, keep_dims=True)+1e-15)
            links.fmat0 = A*drh*((links.dr2[id]**(POW/2-1))*tf.exp(-links.dr2[id]**POW))
        ...
        def dyn_setup(links):
            ...
            links.net.asg['L'] = (links.points.assign_add( f ),)
...
class netRads(netSimple):
    def _init(self):
        self.params.setdefault('nodes',{'radius':self.params['n_radius']}) # for compatibility with networkBase
        self.params.setdefault('long_rep', 0.01) 
        self.params.setdefault('links', {'T0': self.params['nodes']['radius'] / 2.,
                                         'ce': 100.})
        self.get_degrees()
        self.set_radii()
    ...
    def set_radii(self):
        d = self.pts.shape[-1]
        rs = lambda x: ((x**(d-1)).sum(-1))**(1./(d-1)) # use root sum squared of weights as radii for 3D
        r0 = rs(self.Adj)
        self.degreesRS = r0
...
def net_prog(net,steps,ct=0.1, **kw):
    draw_net_tv(net,node_col = '#e82a62', link_col = '#e88f2b', tv=False)
    tt.tic()
    net.iter_all(steps, ct=ct, **kw)
    tt.toc()
...
def get_dl(self, win = 'all'):
    ...
    g0 = gradient(self.tv[-n:])[0].T
    g1 = g0[1]/g0[0]
    l = self.elist.shape [0]
    t,v = array(self.tv[-n:]).T
    return t,g1*t/l

def check_convergence(self, tol = 2, win = 500, **kw):
    t,g1 = get_dl(self,win = win)
    
    #gm = abs(arcsinh(g1)).mean()
    #gm = abs(arcsinh(g1.mean()))
    gm = abs(arcsinh(g1).mean())
    return (gm < tol), gm
        
def run(self,its,ct, draw,**kw):
    if draw:
        return net_prog(self,steps=its, ct=ct, **kw)
    else:
        return self.iter_all(its,ct=ct,**kw)

def iter_converge(self,its = 'auto', max_its = 2e5, draw=True, verbose=True, eps = 0.1, save_path =None, **kw):
    # the average space available per link
    r3 = mean((self.params['nodes']['radius']*self.degrees)**3)**(1./3) # / self.pts.shape[0]
    
    if its=='auto':
        # run 10x the thermal time, till the noise dies
        its = int(10*self.params['links']['ce'])
    run(self,its,eps*r3, draw, **kw)
        
    print "Beginning convergence check ..."
    ck = False
    while (self.it_num < max_its) and not ck:
        if save_path: self.save(save_path,tv=0)
        self.params['long_rep'] = 1e-3 + 100./ (self.it_num+1) 
        ct = eps*max([r3 , self.pts.std()/self.pts.shape[0]**(1./3)]) + 10./(self.it_num+1)
        run(self,its,ct, draw, **kw)
        ck,gm = check_convergence(self,win = its, **kw)
        print '%d) ct=%.2g, '% (self.it_num, ct) + ('' if ck else 'Has Not ') + 'Converged! d<l>/dlog(t)~%.3g' %(gm) 
