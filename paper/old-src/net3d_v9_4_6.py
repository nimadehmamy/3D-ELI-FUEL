...
try:
    ...
    showwarning("Can't import matplotlib.pyplot!", ImportWarning, 'net3d_v9_4_6',8)
...
#                 continue # 
    ...
    def get_ct(self, w= 50, c0 = .1, eps = 1e-3):
        ...
        self.ct = eps+ nan_to_num(c0*max([self.th_mean,self.nrad_mean])**2 /(s1+s+1e-6)) #c0*s1/s + eps
...
def check_convergence(self, tol = .5, win = 500, **kw):
...
def iter_converge(self,its = 'auto', max_its = 2e4, draw=True, verbose=True, eps = 0.1, save_path =None, **kw):
    ...
    while (self.it_num < max_its) and not ck:
        if save_path: self.save(save_path,tv=1)
        ...
        + (' ' if ck else ' Has Not ') + 'Converged! d<l>/dlog(t)~%.3g' %(gm) 
