...
#                 continue # 
    ...
    def get_ct(self, w= 50, c0 = .1, eps = 1e-3):
        i = array(self.tv[-w:])
        if len(i) < w:
            self.ct = c0
            return 0
        ...
        self.ct = eps+ c0*max([self.th_mean,self.nrad_mean]) /(s1+s) #c0*s1/s + eps
    
    def rebin(self, rebin = 'auto' , bin_size = 10, ct = 'auto',c0=0.01, min_step = 40, **kw):
        if rebin == 'auto':
